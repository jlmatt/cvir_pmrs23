---
title: "pmrs_data_assessment_oct"
subtitle:
author: |
...
```{r}
data <- read_xlsx(here::here("data", "raw", "pmar1_survey_data_october_2023.xlsx")) %>%
  janitor::clean_names() %>%
...
```{r}
data<-data %>%
  mutate(tray_num = as.character(NA)) %>%
  mutate(box = as.character(NA))%>%
  mutate(tube_id = as.character(NA)) %>%
  mutate(time_shucked = as.character(NA))
```
...
# Save an RDS version of oct data

```{r}
saveRDS(data,here::here("data","derived","october_data.rds"))
```
