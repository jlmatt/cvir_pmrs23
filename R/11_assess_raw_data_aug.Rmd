---
title: "pmrs_data_assessment_aug"
subtitle:
author: |
...
```{r}
data <- read_xlsx(here::here("data", "raw", "pmar1_survey_data_aug_2024.xlsx")) %>%
  janitor::clean_names()  %>%
...
```{r}
data %>%
  filter(bay %in% c("GAL", "MEG", "MGB", "SAN")) %>%
  filter(!is.na(egg_count))


data %>% filter(bay %in% "GAL|MEG|MGB|SAN") %>% filter(!is.na(egg_count))
```
...
```{r}
saveRDS(data,here::here("data","derived","aug_data.rds"))
```
