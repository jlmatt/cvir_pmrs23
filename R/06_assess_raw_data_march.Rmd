---
title: "pmrs_data_assessment_april"
subtitle:
author: |
...
```{r}
data <- read_xlsx(here::here("data", "raw", "pmar1_survey_data_march_2024.xlsx")) %>%
  janitor::clean_names() %>%
...
```{r}
saveRDS(data,here::here("data","derived","march_data.rds"))
```
...
Make meat weight NA for the one sample with negative meat weight
...
ggplot(fem, aes(sample_site,wt_adj_fec)) + 
  geom_boxplot() + 
   geom_text(data = counts, aes(x = sample_site, y = max(fem$wt_adj_fec), label = paste("n =", count)),
            vjust = 0, hjust = 0.5) + 
    theme_minimal() + 
  ggtitle("March")
```
