...
site_codes<-fec %>%
  ...
  arrange(bay) %>%
  ungroup() %>%
  mutate(code_site = c(1,2,1,2,3,1,2,3,1,1,2,1,2,1,2,1,2)) %>%
  select(-n)
...
site_codes %>%
  filter(sample_site %in% site_info$sample_site)

site_codes <- site_codes %>%
  left_join(.,site_info,by = "sample_site") %>%
  arrange(latitude, .desc = TRUE)
```

# TEST GRAPH

```{r}
ggplot(fec, aes(x = date , y = md, color = fct_reorder(bay,latitude, .desc = TRUE),shape = )) +
  geom_point(size = 5) +
  geom_line() + 
  scale_color_brewer(palette = "RdYlGn",direction = -1)+
